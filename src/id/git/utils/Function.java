...
public class Function {
	...
	public static boolean isSchedule(String startDate, String finishDate) {
		...
		if ((dateNow < Integer.parseInt(startDate))
				|| (dateNow > Integer.parseInt(finishDate))
				|| Integer.parseInt(finishDate) > y) {
			log.warn(printStatus("Not schedule!",
					new Object[]{"Date now", Integer.valueOf(dateNow),
							"Start date", startDate, "Finish date", finishDate,
							"Last Date This month", y}));
			exit();
		}
		return true;
	}
